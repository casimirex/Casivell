...
// Vorsorgepauschale, Beitragsbemessungsgrenze — these are the words printed on the payslip,
// the Steuerbescheid and the Bescheinigung the reader is holding. Rendering "Lohnsteuer" as
// "wage tax" would make a figure *harder* to reconcile against the document it came from,
// which is the one job this tool has.
//
// So `TERMS` is shared by both languages, and only the surrounding prose is translated.
//
...
export interface Messages {
  ...
  heroNet: string;
  heroGross: string;
  heroDeductions: string;
  heroAnnualTax: string;
  heroAllThree: string;
  heroFinalWealth: string;
  heroYears: string;
  heroPension: string;
  loading: string;
  fieldGross: string;
  ...
  fieldYears: string;
  fieldReturn: string;
  fieldPayGrowth: string;
  yes: string;
  no: string;
  explainHint: string;
  why: Record<string, Why>;
  captionPayslip: string;
  captionClasses: string;
  captionProject: string;
  explainShow: string;
  perMonth: string;
  ...
  inForce: string;
  legalBasis: string;
  announceNet: (net: string, gross: string) => string;
  classesLede: (total: string) => string;
  colVariant: string;
  colHigher: string;
  colLower: string;
  colNetTotal: string;
  colAssessment: string;
  owes: (amount: string) => string;
  back: (amount: string) => string;
  noFactor: string;
  whySameTitle: string;
  whySame: string;
  whyMattersTitle: string;
  whyMatters: string;
  announceLiability: (total: string) => string;
  colYear: string;
  colNetMonthly: string;
  colSaved: string;
  colWealth: string;
  colPension: string;
  whatsHereTitle: string;
  whatsHere: (last: string | number) => string;
  turnsTitle: string;
  turns: (peak: string, year: number) => string;
  missingTitle: string;
  missing: string;
  chartCaption: string;
  chartLabel: (from: number, to: number, max: string) => string;
  announceProjection: (years: number, wealth: string, pension: string) => string;
...
export const MESSAGES: Record<Lang, Messages> = {
  de: {
    lang: "de",
    tagline:
      "Brutto zu Netto, gerechnet nach dem BMF-Programmablaufplan. " +
    ...
    copied: "Kopiert",
    heroNet: "Nettolohn",
    heroGross: "Bruttolohn",
    heroDeductions: "Abzüge",
    heroAnnualTax: "Jahres-Einkommensteuer",
    heroAllThree: "bei allen drei Varianten",
    heroFinalWealth: "Endvermögen",
    heroYears: "Jahre",
    heroPension: "Rente/Monat",
    loading: "Laden…",
    ...
    why: {
      incomeTax: {
        ...
        text:
          "Der PAP rechnet den Monatslohn aufs Jahr hoch, zieht die Freibeträge und die " +
      ...
      solidarity: {
        law: "SolzG 1995 · § 3, § 4",
        text:
          "5,5 % der Lohnsteuer, aber erst oberhalb der Freigrenze und dann zunächst " +
      ...
      churchTax: {
        ...
        text:
          "8 % in Bayern und Baden-Württemberg, 9 % sonst — bemessen auf die Lohnsteuer, " +
      ...
      health: {
        law: "§ 241 SGB V",
        text:
          "14,6 % geteilt durch zwei, plus die Hälfte des kassenindividuellen " +
          "Zusatzbeitrags.",
      },
      care: {
        law: "§ 55 SGB XI",
        text:
          "3,6 % geteilt durch zwei, mit Zuschlag für Kinderlose ab 23 und Abschlag je " +
      ...
      net: {
        law: "—",
        text:
          "Brutto abzüglich aller Posten darüber. Vorläufig: der Jahresausgleich rechnet ab.",
      },
    },
    ...
    updateAvailable:
      "Eine neuere Fassung ist verfügbar — möglicherweise mit geänderten " +
      "Rechtsständen.",
    reload: "Neu laden",
    ...
    offline:
      "Funktioniert offline. Der Rechtsstand oben gilt dann für die zuletzt geladene " +
    ...
    whyMatters:
      "Lohnersatzleistungen werden aus dem <em>Netto</em> berechnet: Elterngeld, " +
    ...
    colYear: "Jahr",
    colNetMonthly: "Netto/mtl.",
    colSaved: "Sparrate",
    colWealth: "Vermögen",
    colPension: "Rente/mtl.",
    whatsHereTitle: "Was hier steht",
    whatsHere: (last) =>
      `Jede Zeile ist ein Jahresende. Mit <strong>*</strong> markierte Jahre ` +
    ...
    turns: (peak, year) =>
      `Der höchste Stand liegt bei ${peak} im Jahr ${year}, danach fällt ` +
    ...
    missing:
      "Immobilien, Elterngeld, Kapitalerträge und Lebensereignisse rechnet nur die " +
      "Kommandozeile. Renditen sind Annahmen, keine Zusagen.",
    chartCaption:
      "Geldvermögen, nominal. Gestrichelt: Jahre auf fortgeschriebenen " +
      "Rechengrößen.",
    chartLabel: (from, to, max) =>
      `Vermögensverlauf von ${from} bis ${to}, Höchststand ${max}`,
    ...
    errors: {
      ...
      "-4":
        "Eine Eingabe liegt außerhalb des zulässigen Bereichs. Kinder setzen die " +
  ...
  en: {
    lang: "en",
    tagline:
      "Gross to net, computed from the BMF Programmablaufplan. Nothing leaves this " +
      "device.",
    ...
    heroNet: "Net pay",
    heroGross: "Gross pay",
    heroDeductions: "Deductions",
    heroAnnualTax: "Annual income tax",
    heroAllThree: "under all three arrangements",
    heroFinalWealth: "Final wealth",
    heroYears: "Years",
    heroPension: "Pension/month",
    loading: "Loading…",
    ...
    why: {
      incomeTax: {
        law: "§ 39b EStG · BMF Programmablaufplan",
        text:
          "The PAP annualises the monthly wage, deducts the table allowances and the " +
      ...
      solidarity: {
        law: "SolzG 1995 · § 3, § 4",
        text:
          "5.5 % of the Lohnsteuer — but only above the Freigrenze, and tapered at first. " +
          "Below it there is none, which is why most salaries show 0,00 € here.",
      },
      churchTax: {
        law: "Kirchensteuergesetze der Länder · § 51a EStG",
        text:
          "8 % in Bayern and Baden-Württemberg, 9 % elsewhere — levied on the Lohnsteuer " +
      ...
      pension: {
        law: "§ 158 SGB VI",
        text:
          "18.6 % halved between employer and employee, up to the " +
      ...
      care: {
        law: "§ 55 SGB XI",
        text:
          "3.6 % halved, with a surcharge for the childless from age 23 and a reduction " +
      ...
      net: {
        law: "—",
        text:
          "Gross less everything above it. Provisional: the annual assessment settles the " +
          "difference.",
      },
    },
    ...
    offline:
      "Works offline. The statutory basis shown above is then the one the cached " +
    ...
    whyMatters:
      "Wage-replacement benefits are computed from <em>net</em> pay: Elterngeld, " +
    ...
    colYear: "Year",
    colNetMonthly: "Net/month",
    colSaved: "Saved/month",
    colWealth: "Wealth",
    colPension: "Pension/month",
    whatsHereTitle: "What this shows",
    whatsHere: (last) =>
      `Each row is a year end. Years marked <strong>*</strong> rest on ` +
      `projected statutory figures rather than enacted law — beyond ${last} nothing has been ` +
      `enacted.`,
    turnsTitle: "The wealth line turns",
    turns: (peak, year) =>
      `It peaks at ${peak} in ${year} and falls thereafter. When spending ` +
      `rises with prices and pay does not, spending eventually overtakes net pay.`,
    missingTitle: "What is not included",
    missing:
      "Property, Elterngeld, capital income and life events are computed only by the " +
...
// The language to start in: an explicit `?lang=`, else a stored choice, else the
// browser's, else German. The URL parameter comes first so a link can carry its
// language; German is the fallback because the figures, the terms and the
// documents they are compared against are all German.
export function initialLanguage(
  stored: string | null,
  navigatorLanguages: readonly string[] = [],
  requested: string | null = null,
): Lang {
  if (requested && MESSAGES[requested as Lang]) return requested as Lang;
  if (stored && MESSAGES[stored as Lang]) return stored as Lang;
  for (const tag of navigatorLanguages) {
    const base = tag.slice(0, 2).toLowerCase();
    if (MESSAGES[base as Lang]) return base as Lang;
  }
  return "de";
}
